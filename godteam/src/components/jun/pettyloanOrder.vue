    ...
    <div style="width: 100%;height: 100%">
        ...
        <el-tabs v-model="activeName" @tab-click="handleClick">
            <el-tab-pane label="贷款订单" name="first">
                ...
                <div style="margin:0 auto;width: 90%;height: 93%;border: red solid 0px">
                    <div v-for="petty in pettyloan" style="float: left;font-size: 14px;height: 80px;margin-top: 18px;margin-bottom:1%;width: 100%;margin-right: 1%;border-radius:8px 8px 8px 8px; box-shadow:#F8F8FF 2px 2px 5px 5px" class="tb1">
                        <table style="margin-left:3%;width:100%;height:80px;border-collapse: collapse;">
                            <tr>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px">订单编号</td>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px">贷款金额</td>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px">分几期还款</td>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px">贷款时间</td>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px">已还几期</td>
                                <td style="border-bottom: rgba(180,173,163,0.85) dashed 1px ;width: 90px">每期应还金额</td>
                                <td rowspan="2" style="text-align: center">
                                    <span v-show="petty.mou!=petty.yihuan">
                                        <el-tag type="warning">
                                        还款中
                                    </el-tag><br/><el-link type="primary" @click="selectinfo(petty.sdoid,petty.daimoney,petty.mou,petty.onemoney,)">提前还款</el-link>
                                    </span>
                                    <span v-show="petty.mou==petty.yihuan">
                                        <el-tag type="success">
                                        已还完
...
<script>
    export default {
        ...
        mounted() {
            this.axios({
                url:"http://localhost:10086/selectSaollOrder",
                method:"POST",
            ...
            this.axios({
                url:"http://localhost:10086/selectsmallhuankuan",
                method:"POST",
                withCredentials: true,
                data:{}
            }).then(res=>{
                res.data.forEach(ele=>{
                    ...
                    let M=(date.getMonth()+1);
                    let d=date.getDate();
                    this.smallhuankuan.push({"hid":ele.hid,"sdoid":ele.sdoid,"liushui":ele.liushui,"zhifutype":ele.zhifutype,"money":ele.money,"huanDate":y+"-"+M+"-"+d})
                })
            })
        },
        methods:{
...
this.pageno=val;
            ...
            fukuan(){
                ...
                this.axios({
                    url:"http://localhost:10086/addrepayment",
                    method:"POST",
                    withCredentials: true,
                    data:{
                        id:this.id,
           ...
           //点击订单查到订单信息并循环显示期数
            selectinfo(id,dai,daimou,daililv){
                ...
                this.axios({
                    url:"http://localhost:10086/selectSaollOrderByid",
                    method:"POST",
                    withCredentials: true,
                    data:{
                        id:id
